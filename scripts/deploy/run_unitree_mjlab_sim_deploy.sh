...
GO2_UNITREE_TERRAIN_SCENE="${REPO_ROOT}/reference_repos/unitree_mujoco/unitree_robots/go2/scene_terrain.xml"
MUJOCO_PYTHON="${MUJOCO_PYTHON:-python}"
TERRAIN_DIR="${REPO_ROOT}/artifacts/mujoco_mjlab_terrains"
...
Active runtime selectors:
  asym      -> go2_blind_rough_asymppo_mjlab_v1_candidate
  combined  -> go2_blind_rough_combined_asymppo_steps_v1_candidate
...
case "${1:-}" in
  sim)
    ...
    ;;
  sim-unitree-terrain)
    if [[ ! -f "${GO2_UNITREE_TERRAIN_SCENE}" ]]; then
      echo "Missing Unitree terrain scene: ${GO2_UNITREE_TERRAIN_SCENE}" >&2
      exit 1
    fi
    printf '%s\n' \
      "NOTE: Loading the stock Unitree MuJoCo terrain scene." \
      "This contains rough boxes, stairs, slope, and heightfields." \
      "Use 'sim-terrain' for MJLAB procedural terrain once the mjlab terrain package is available."
    exec "${SIM_BIN}" \
      --network=lo \
      --robot=go2 \
      --scene="${GO2_UNITREE_TERRAIN_SCENE}" \
      --use_joystick=0
    ;;
  sim-terrain)
    ...
    if [[ ! -f "${scene}" ]]; then
      "${MUJOCO_PYTHON}" \
        "${REPO_ROOT}/scripts/deploy/materialize_mjlab_mujoco_terrain.py" \
  ...
  activate)
    ...
    case "${selector}" in
      asym)
        bundle="${REPO_ROOT}/rma_go2_lab/policies/exported/go2_blind_rough_asymppo_mjlab_v1_candidate"
        runtime_name="go2_blind_rough_asymppo_mjlab_v1_candidate"
        ;;
      combined)
        bundle="${REPO_ROOT}/rma_go2_lab/policies/exported/go2_blind_rough_combined_asymppo_steps_v1_candidate"
        runtime_name="go2_blind_rough_combined_asymppo_steps_v1_candidate"
        ;;
      *)
        echo "Unknown runtime selector: ${selector}" >&2
        echo "Expected one of: asym, combined" >&2
        exit 2
        ;;
    esac
    exec "${MUJOCO_PYTHON}" \
  ...
  robot-wifi-connect)
    ...
    exec "${REPO_ROOT}/scripts/deploy/configure_go2_wifi_over_ssh.sh" \
  ...
  dds-probe)
    ...
    if "${go2_hw_python}" "${REPO_ROOT}/scripts/deploy/probe_go2_readonly.py" \
      --net-if "${network}" \
      --duration 3 \
      --print-every 1 \
      --no-sport; then
  ...
  monitor)
    ...
    network="$(go2_resolve_network_interface "${selector}" "${explicit_interface}")"
    go2_validate_network_interface "${network}"
    exec "${REPO_ROOT}/scripts/deploy/run_go2_realtime_monitor.sh" \
      "${network}" 20 25 "${label}"
    ;;
  hardware)
    selector="${2:-ethernet}"
