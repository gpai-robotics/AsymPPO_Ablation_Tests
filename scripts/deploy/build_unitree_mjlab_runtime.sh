...
Typical recovery flow after a clean checkout or broken deploy path:
  1. bash scripts/deploy/build_unitree_mjlab_runtime.sh all
  2. bash scripts/deploy/run_unitree_mjlab_sim_deploy.sh activate
  3. bash scripts/deploy/run_unitree_mjlab_sim_deploy.sh validate
