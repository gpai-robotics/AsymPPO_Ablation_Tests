...
class Go2ReadOnlyProbe:
    ...
    def _low_snapshot(self) -> dict[str, Any] | None:
        ...
        tau_est = [float(self.low_state.motor_state[i].tau_est) for i in range(12)]
    ...
    def _json_safe_value(self, value: Any, depth: int = 0) -> Any:
        """Best-effort serializer for Unitree IDL objects.

        This is intentionally conservative: it records primitive public fields
        and avoids callables/private internals. The result is for diagnostics,
        not for round-tripping DDS messages.
        """
        if depth > 4:
            return repr(value)
        ...
        if isinstance(value, list | tuple):
            return [self._json_safe_value(item, depth + 1) for item in value]
        if isinstance(value, dict):
            return {
                str(key): self._json_safe_value(item, depth + 1)
        ...
        for name in dir(value):
            ...
            if callable(attr):
                continue
            try:
                fields[name] = self._json_safe_value(attr, depth + 1)
            except Exception as exc:
                fields[name] = f"<unserializable:{type(exc).__name__}>"
        if fields:
            return fields
        return repr(value)

    def _blueprint_snapshot(self) -> dict[str, Any]:
        """Return a broad read-only dump of currently visible DDS fields."""
        return {
            "schema": "go2_readonly_blueprint_v1",
            "note": "Best-effort public-field dump from Unitree DDS IDL objects.",
            "lowstate": self._json_safe_value(self.low_state) if self.low_state is not None else None,
            "sportmodestate": (
                self._json_safe_value(self.sport_state) if self.sport_state is not None else None
            ),
            "lowcmd": self._json_safe_value(self.low_cmd) if self.low_cmd is not None else None,
        }
    ...
    def run(
        ...
        try:
            ...
            if include_blueprint:
                result["blueprint"] = self._blueprint_snapshot()
            return result
        finally:
            if self._lowstate_stream_handle is not None:
                with self._lowstate_stream_lock:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--include-blueprint",
        action="store_true",
        help="Include a broad best-effort dump of public LowState/SportModeState/LowCmd fields in the final JSON.",
    )
