"""Debug and validate Adapt-V2 before long training.

This script validates the explicitly modular V2 student path:

- env/task registration and observation groups
- adaptation-module output shape and stats
- explicit base-policy path `pi(x_t, z_hat_t)`
- frozen teacher latent target shape and stats
- masked latent regression signal on real env observations
...
parser = argparse.ArgumentParser(description="Inspect and validate Adapt-V2 modular latent wiring.")
parser.add_argument("--task", type=str, default="RMA-Go2-Adaptation-Student-Rough-History-V2")
...
from rma_go2_lab.models.adaptation.adapt_v2_ppo_cfg import Go2AdaptationStudentV2PPORunnerCfg
...
def _build_student(obs_td: TensorDict, runner_cfg: Go2AdaptationStudentV2PPORunnerCfg, device: torch.device):
    policy_cfg = runner_cfg.policy
    student = ModularHistoryAdaptationActorCritic(
        obs=obs_td,
...
def _compute_batch_debug(
    ...
    with torch.no_grad():
        ...
        modular_actions = student.act_with_latent(obs_td["policy"], student_latent)
        ...
        modular_delta_per_env = ((student_actions - modular_actions) ** 2).mean(dim=-1)
        cosine_per_env = torch.nn.functional.cosine_similarity(student_latent, teacher_latent, dim=-1)

        if torch.count_nonzero(mask) > 0:
            denom = mask.sum().item() + 1e-6
            masked_latent_mse = float((latent_mse_per_env * mask).sum().item() / denom)
            masked_latent_cosine = float((cosine_per_env * mask).sum().item() / denom)
            masked_action_mse = float((action_mse_per_env * mask).sum().item() / denom)
            masked_modular_delta = float((modular_delta_per_env * mask).sum().item() / denom)
        else:
            masked_latent_mse = 0.0
            masked_latent_cosine = 0.0
            masked_action_mse = 0.0
            masked_modular_delta = 0.0
    ...
    return {
        ...
        "modular_action_stats": _tensor_stats(modular_actions),
        ...
        "masked_modular_action_delta": masked_modular_delta,
