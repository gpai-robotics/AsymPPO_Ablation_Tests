...
ISAACLAB_ROOT="${ISAACLAB_ROOT:-/home/bhuvan/tools/IsaacLab}"
...
if [[ ! -x "${ISAACLAB_SH}" ]]; then
    ...
    echo "Set ISAACLAB_ROOT to the shared IsaacLab checkout." >&2
    exit 1
fi
...
if [[ -d "${ML_ARCHIVE}" ]]; then
    # PyTorch CUDA wheels bundled with Isaac Sim keep runtime libraries under
    # package-local lib directories. Expose them explicitly so shared /opt
    # installs do not fail on libraries such as libcusparseLt.so.0 or libnccl.so.2.
