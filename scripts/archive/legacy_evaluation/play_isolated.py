...
parser.add_argument(
    ...
    default=None,
    help=(
        "Terrain selector. Supports a concrete terrain name (e.g. pyramid_stairs, boxes) "
        "and eval-only presets like mixed_all or mixed_geom_ood."
    ),
)
...
parser.add_argument(
    ...
    type=str,
    default="task",
    choices=["task", "standstill", "forward", "random_per_env_fixed", "random_per_env_resample"],
...
parser.add_argument(
    "--random-command-resample-steps",
    type=int,
    default=150,
    help="When using --command-profile random_per_env_resample, resample every N sim steps.",
)
...
def _force_isolated_terrain(env_cfg, terrain_type: str | None) -> None:
    ...
    if terrain_type == "mixed_all":
        ...
        missing = [name for name in preset if name not in terrain_gen.sub_terrains]
        if missing:
            valid = list(terrain_gen.sub_terrains.keys())
            raise ValueError(
                f"Terrain preset 'mixed_all' requires missing terrains {missing}. Valid options: {valid}"
            )
        for name in preset:
            terrain_gen.sub_terrains[name].proportion = 1.0
        return
    if terrain_type == "mixed_geom_ood":
        preset = [
            "boxes",
            "pyramid_stairs",
            "pyramid_stairs_inv",
        ...
        if missing:
            valid = list(terrain_gen.sub_terrains.keys())
            raise ValueError(
                f"Terrain preset 'mixed_geom_ood' requires missing terrains {missing}. Valid options: {valid}"
            )
        for name in preset:
            terrain_gen.sub_terrains[name].proportion = 1.0
        return
...
def _set_forced_velocity_command(env, command: torch.Tensor, env_ids: torch.Tensor | None = None) -> None:
    command_term = env.unwrapped.command_manager.get_term("base_velocity")
    if env_ids is None:
        env_ids = torch.arange(env.num_envs, device=command_term.vel_command_b.device)
    command_term.vel_command_b[env_ids, :] = command[env_ids, :]
    command_term.time_left[env_ids] = 1.0e6
    command_term.command_counter[env_ids] = 1
    if hasattr(command_term, "is_heading_env"):
        command_term.is_heading_env[env_ids] = False
    if hasattr(command_term, "is_standing_env"):
        command_term.is_standing_env[env_ids] = False
    if hasattr(command_term, "heading_target"):
        command_term.heading_target[env_ids] = env.unwrapped.scene["robot"].data.heading_w[env_ids]


def _get_base_velocity_ranges(env):
    command_term = env.unwrapped.command_manager.get_term("base_velocity")
    ranges = getattr(command_term.cfg, "limit_ranges", None)
    if ranges is None:
        ranges = command_term.cfg.ranges
    return ranges


def _sample_random_per_env_command(env) -> torch.Tensor:
    robot = env.unwrapped.scene["robot"]
    ranges = _get_base_velocity_ranges(env)
    command = torch.empty((env.num_envs, 3), device=robot.device, dtype=robot.data.root_lin_vel_b.dtype)
    command[:, 0].uniform_(*ranges.lin_vel_x)
    command[:, 1].uniform_(*ranges.lin_vel_y)
    command[:, 2].uniform_(*ranges.ang_vel_z)
    return command
...
def _forced_command(env) -> torch.Tensor | None:
    ...
    if args_cli.command_profile == "forward":
        ...
        return command
    if args_cli.command_profile in {"random_per_env_fixed", "random_per_env_resample"}:
        return _sample_random_per_env_command(env)
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            if teleop is not None:
                ...
                if (
                    args_cli.command_profile == "random_per_env_resample"
                    and args_cli.random_command_resample_steps > 0
                    and step_count > 0
                    and step_count % args_cli.random_command_resample_steps == 0
                ):
                    forced_command = _forced_command(env)
                _set_forced_velocity_command(env, forced_command)
            actions = policy(obs)
            ...
            if isinstance(dones, torch.Tensor) and bool(dones.any().item()):
                ...
                env.unwrapped.reset(env_ids=done_env_ids)
                _force_terrain_level(env, args_cli.terrain_level)
                if forced_command is not None:
                    if args_cli.command_profile in {"random_per_env_fixed", "random_per_env_resample"}:
                        forced_command[done_env_ids] = _sample_random_per_env_command(env)[done_env_ids]
                    _set_forced_velocity_command(env, forced_command, env_ids=done_env_ids)
