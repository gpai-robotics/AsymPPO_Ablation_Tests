...
class AdaptationPPOWithV3Latent(PPO):
    """PPO with explicit latent regression to the frozen V3 teacher target.

    This is the first separated Adapt-V1 path:

    - student predicts `z_hat_t` from history
    - frozen teacher exports `z_target`
    - optimization uses PPO + latent regression
    - optional weak action imitation can remain enabled early in training
    """
    ...
    def update(self) -> dict[str, float]:
        ...
        for (
            ...
            required_groups = {"policy", "policy_history", "terrain_privileged", "dynamics_privileged"}
            if self.v3_expert is not None and required_groups.issubset(set(obs_batch.keys())):
                expert_obs_batch = obs_batch[:original_batch_size]
                latent_mask = self._command_mask(expert_obs_batch, self.latent_command_threshold)
                if latent_mask is not None:
                    latent_active_frac = latent_mask.mean()
                    if torch.count_nonzero(latent_mask) > 0:
                        student_latent = self.policy.encode_history_latent(expert_obs_batch)
                        teacher_latent = self.v3_expert.get_latent_target(expert_obs_batch).detach()
                        if student_latent.shape != teacher_latent.shape:
                            raise RuntimeError(
                                "Adapt-V1 latent shape mismatch: "
                ...
                if teacher_imitation_coef > 0.0:
                    imitation_mask = self._command_mask(expert_obs_batch, self.imitation_command_threshold)
