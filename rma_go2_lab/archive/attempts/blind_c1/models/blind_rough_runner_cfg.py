...
OMNI_FLAT_EXPERT_CKPT = "/home/bhuvan/projects/rma/rma_go2_lab/rma_go2_lab/policies/flat_omni_v1.pt"
ROUGH_OMNI_TEACHER_V1_CKPT = "/home/bhuvan/projects/rma/rma_go2_lab/rma_go2_lab/policies/rough_omni_teacher_v1.pt"
...
def _resolve_c1_omni_warmstart() -> str:
    """Prefer an omni flat prior once it exists, otherwise fall back safely."""
    if Path(OMNI_FLAT_EXPERT_CKPT).exists():
        return OMNI_FLAT_EXPERT_CKPT
    return FLAT_EXPERT_CKPT
...
@configclass
class Go2BlindRoughForwardScratchPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    num_envs = None
...
@configclass
class Go2BlindRoughForwardWarmStartPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    num_envs = None
    num_steps_per_env = 32
    max_iterations = 2000
    save_interval = 20
...
@configclass
class Go2BlindRoughForwardWarmStartImitationPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    num_envs = None
    num_steps_per_env = 32
    max_iterations = 2000
    save_interval = 20
...
@configclass
class Go2C1BlindRoughTeacherHistoryV1PPORunnerCfg(RslRlOnPolicyRunnerCfg):
    num_envs = None
    num_steps_per_env = 32
    max_iterations = 2000
    save_interval = 20
...
@configclass
class Go2C1BlindRoughTeacherHistoryV2PPORunnerCfg(Go2C1BlindRoughTeacherHistoryV1PPORunnerCfg):
...
@configclass
class Go2C1BlindRoughTeacherHistoryV3PPORunnerCfg(Go2C1BlindRoughTeacherHistoryV1PPORunnerCfg):
...
@configclass
class Go2C1BlindRoughOmniV1PPORunnerCfg(Go2C1BlindRoughTeacherHistoryV3PPORunnerCfg):
    """First C1 omnidirectional retrain on top of the current StageA line."""

    experiment_name = "go2_c1_omni_v1_stagea"
    ...
    policy = BlindHistoryPolicyCfg(
        ...
        critic_hidden_dims=[512, 256, 128],
        activation="elu",
        actor_init_path=_resolve_c1_omni_warmstart(),
        history_group_name="policy_history",
        temporal_channels=[64, 64],
        temporal_kernel_size=3,
        history_feature_dim=64,
...
@configclass
class Go2C1BlindRoughOmniSimpleV1PPORunnerCfg(Go2C1BlindRoughOmniV1PPORunnerCfg):
    """Canonical simple rough-terrain omni candidate on top of the omni flat prior."""

    experiment_name = "go2_c1_blind_rough_omni_simple_v1"


@configclass
class Go2C1BlindRoughOmniUsableV1PPORunnerCfg(Go2C1BlindRoughOmniSimpleV1PPORunnerCfg):
    """Usable-range rough omni candidate with a narrower target envelope."""

    experiment_name = "go2_c1_blind_rough_omni_usable_v1"

    # Keep the same branch identity and training philosophy, but ease the
    # student into the much stronger omni teacher. The flat-omni warm start
    # should first re-stabilize rough locomotion before heavier teacher pull.
    algorithm = BlindV3TeacherImitationAlgorithmCfg(
        value_loss_coef=1.0,
        use_clipped_value_loss=True,
        clip_param=0.2,
        ...
        lam=0.95,
        max_grad_norm=1.0,
        v3_expert_path=ROUGH_OMNI_TEACHER_V1_CKPT,
        latent_command_threshold=0.1,
        latent_regression_coef_stage0=0.05,
        latent_regression_coef_stage1=0.02,
        latent_regression_coef_stage2=0.01,
        latent_stage0_end=400,
        latent_stage1_end=1000,
        imitation_command_threshold=0.1,
        imitation_coef_stage0=0.02,
        imitation_coef_stage1=0.005,
        imitation_stage0_end=400,
        imitation_stage1_end=1000,
    )
