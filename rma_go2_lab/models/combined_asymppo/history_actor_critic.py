...
class TemporalBlindActorCritic(ActorCritic):
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        print(f"Temporal encoder: {self.temporal_encoder}")
        print(f"History projection: {self.history_projection}")
        print(f"History target head: {self.history_target_head}")

        actor_non_history_dim = 0
        for obs_group in obs_groups["policy"]:
            assert len(obs[obs_group].shape) == 2, "ActorCritic only supports 1D observations."
    ...
    def load_actor_only(self, checkpoint_path: str) -> None:
        ...
        print(
            f"[INFO] Warm-started temporal blind actor from {checkpoint_path}. "
            "History pathway keeps small non-zero initialization so it remains trainable."
        )
