...
class BlindPPOWithV3Teacher(PPO):
    """PPO with a time-decayed imitation prior from the frozen V3 teacher."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if v3_expert_path:
            self.v3_expert = FrozenV3Expert(
                checkpoint_path=v3_expert_path,
                device=self.device,
            ).to(self.device)
    ...
    def _imitation_mask(self, obs_batch) -> torch.Tensor | None:
        if self.v3_expert is None or "policy" not in obs_batch.keys():
            return None
        command = obs_batch["policy"][:, 9:12]
        return (torch.linalg.norm(command, dim=-1) > self.imitation_command_threshold).float()

    def _latent_mask(self, obs_batch) -> torch.Tensor | None:
        if self.v3_expert is None or "policy" not in obs_batch.keys():
            return None
        command = obs_batch["policy"][:, 9:12]
        return (torch.linalg.norm(command, dim=-1) > self.latent_command_threshold).float()
    ...
    def update(self) -> dict[str, float]:
        ...
        for (
            ...
            if self.v3_expert is not None:
                expert_obs_batch = obs_batch[:original_batch_size]
                required_groups = {"policy", "terrain_privileged", "dynamics_privileged"}
                if required_groups.issubset(set(expert_obs_batch.keys())):
                    if latent_regression_coef > 0.0 and hasattr(self.policy, "encode_history_target"):
                        latent_mask = self._latent_mask(expert_obs_batch)
                        if latent_mask is not None:
                            latent_active_frac = latent_mask.mean()
                            if torch.count_nonzero(latent_mask) > 0:
                                student_target = self.policy.encode_history_target(expert_obs_batch)
                                teacher_target = self.v3_expert.get_latent_target(expert_obs_batch).detach()
                                if student_target.shape != teacher_target.shape:
                                    raise RuntimeError(
                                        "Blind history target shape mismatch: "
                                        f"student={tuple(student_target.shape)} teacher={tuple(teacher_target.shape)}"
                                    )
                                ...
                                cosine = nn.functional.cosine_similarity(student_target, teacher_target, dim=-1)
                    ...
                    if imitation_coef > 0.0:
                        ...
                        if mask is not None:
                            ...
                            if torch.count_nonzero(mask) > 0:
                                teacher_actions = self.v3_expert(expert_obs_batch).detach()
            ...
            mean_latent_regression_coef += latent_regression_coef
        ...
        mean_latent_regression_coef /= num_updates
        ...
        loss_dict = {
            ...
            "history_target_regression": mean_latent_regression_loss,
            "history_target_active_frac": mean_latent_active_frac,
            "history_target_regression_coef": mean_latent_regression_coef,
            "history_target_cosine": mean_latent_cosine,
            "teacher_imitation": mean_imitation_loss,
            "teacher_imitation_active_frac": mean_imitation_active_frac,
            "teacher_imitation_coef": mean_imitation_coef,
        }
        if self.rnd:
