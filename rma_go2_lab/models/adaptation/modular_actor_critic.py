...
class ModularHistoryAdaptationActorCritic(ActorCritic):
    """V2 scaffold with an explicit RMA-like module split.

    This keeps compatibility with the current PPO stack while making the
    deployment-time decomposition explicit:

    - adaptation module phi:
        history -> z_hat_t
    - base policy pi:
        current deployable obs + z_hat_t -> action / value

    The implementation is still synchronous inside one PyTorch module, but the
    responsibilities are separated so later deployment code can run the two
    paths at different rates.
    """
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        if kwargs:
            print(
                "ModularHistoryAdaptationActorCritic.__init__ got unexpected arguments, which will be ignored: "
        ...
        if not (self._actor_has_history or self._critic_has_history):
            raise ValueError(
                f"ModularHistoryAdaptationActorCritic expects observation group '{history_group_name}' "
        ...
        assert len(current_policy_obs.shape) == 2, "Current policy observations must be 1D per environment."
        current_policy_dim = current_policy_obs.shape[-1]

        self.adaptation_module = MLP(history_dim, latent_dim, adaptation_hidden_dims, activation)
        ...
        print(f"Base policy critic: {self.critic}")
    ...
    def adapt_from_history(self, history_obs: torch.Tensor) -> torch.Tensor:
        """Explicit adaptation path: phi(history) -> z_hat_t."""
    ...
    def act_with_latent(self, current_policy_obs: torch.Tensor, z_hat: torch.Tensor) -> torch.Tensor:
        """Explicit deployment path: pi(x_t, z_hat_t) -> action mean."""
        actor_obs = torch.cat([current_policy_obs, z_hat], dim=-1)
    ...
    def load_actor_only(self, checkpoint_path: str) -> None:
        ...
        self._small_init_module(self.adaptation_module)
        self._partial_copy_mlp(self.actor, actor_state)
        print(
            f"[INFO] Warm-started V2 base-policy actor from {checkpoint_path}. "
            "Adaptation module keeps small non-zero initialization so it remains trainable."
        )
