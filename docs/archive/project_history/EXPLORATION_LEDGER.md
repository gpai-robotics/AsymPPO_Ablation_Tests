...
| --- | --- | --- | --- | --- | --- |
| 2026-05-21 | C1 deployment | frozen | `c1_blind_rough_omni_usable_v1_final deploy probes` | First deployment-facing hidden-dynamics probe pass plus flat-surface MuJoCo sanity pass for the frozen rough omni student | Export parity, Isaac deploy rehearsal, and nominal flat-surface MuJoCo replay are clean; low friction is the first trusted failure case, while tested mass and motor shifts are secondary |
| 2026-05-21 | C1 | frozen | `c1_blind_rough_omni_usable_v1_final.pt` | First cleaned deployable blind rough omni student built on the omni flat prior and frozen omni rough teacher with a practical usable command envelope | Canonical rough omni deployment candidate; closes the omni training stack for now and becomes the immediate export / deployment / sim2real artifact |
| 2026-05-21 | C1 | frozen | `rough_omni_teacher_v1.pt` | First coherent privileged rough omni teacher built on the omni flat prior with terrain + dynamics privilege and full omni command curriculum | Canonical omni teacher artifact; replaces the forward-teacher mismatch for omni student work and becomes the upstream supervisor for the active deployable rough omni student |
