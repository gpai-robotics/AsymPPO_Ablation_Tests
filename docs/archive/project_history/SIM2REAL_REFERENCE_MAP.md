...
To avoid wasting time on the wrong deployment framework, the reference repos
should be read as solving different layers of the problem, not as competing
full-stack answers.
...
- keep this repo as the source of truth for:
  ...
  - deployable observation/action contract
  - Isaac deployment rehearsal and controlled robustness probes
- treat `reference_repos/unitree_rl_lab/deploy` as the long-term robot-runtime
  architecture target:
  - explicit FSM
  - robot-side ONNX inference
  - generated deploy config beside the exported policy
- treat `reference_repos/sim2real_unitree_sdk2py` as the operational safety and
  bring-up shell:
  - DDS wiring
  - zero-torque / default-stance / operator-gated start flow
  - conservative stop semantics
- treat `reference_repos/mujoco_playground` as a replay/runtime reference only:
  - useful for ONNX and viewer patterns
  - not the hardware deployment framework to copy wholesale

Short version:

- do not pivot the project toward a `mujoco_playground`-style deployment stack
- do not revive a standalone `sim2real_unitree_sdk2py` policy contract as the
  primary truth
- do preserve compatibility toward a `unitree_rl_lab`-style
  `policy.onnx + deploy.yaml + FSM` end-state
