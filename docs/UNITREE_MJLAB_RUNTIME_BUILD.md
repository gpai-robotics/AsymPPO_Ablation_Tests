...
Both live under `reference_repos/unitree_rl_mjlab/` and are launched by
...
The materializer bridge must expose `materialize_bundle()` in
`scripts/deploy/materialize_unitree_rl_lab_layout.py`. If `activate` fails with
`ImportError: cannot import name 'materialize_bundle'`, restore that function
from `scripts/archive/legacy_deployment/materialize_unitree_rl_lab_layout.py`.
